...
{
    // TODO
   //  printf("entra aqui");  printf("entra aqui");
   
    char* result = malloc (strlen(path)+11);
    if(result==NULL){
        return NULL;
    }
      strcpy(result,path);
    ...
    //to by the path argument for accessibility according to the bit pattern contained 
    //in amode, using the real user ID in place of the effective user ID and the real 
    //group ID in place of the effective group ID.
    //The value of amode is either the bitwise-inclusive OR of the access permissions 
    //to be checked (R_OK, W_OK, X_OK) or the existence test (F_OK).
     int existe1=  access(result, F_OK);
     if( existe1 != -1){
      //   printf("bien html");
       return result;
     ...
     char* result1 = malloc (strlen(path)+10);
    if(result1==NULL){
        return NULL;
    }
      strcpy(result1,path);
      strcat(result1,"index.php");
    
   
     int existe2=  access(result1, F_OK);
     if( existe2 != -1){
       return result1;
     }else{
        free(result1);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    size_t bytes_read = 0;
    while ((bytes_read = fread(buffer, 1, BYTES, file)) > 0)
    {
        // increment length and reallocate memory for content
        *length+= bytes_read;

        if ((*content = realloc(*content, *length)) == NULL)
        {
            return false;
        }

        // write buffer contents to **content
        memcpy(*content + *length - bytes_read, &buffer, bytes_read);
    }
    return true;
}
...
//     char extension[strlen(ext) + 1];
//     strcpy(extension, ext);
//     char ini[strlen(inic) + 1];
//     strcpy(ini, inic);
    
//     extension[strlen(ext)] = '\0';
//     ini[strlen(inic)] = '\0';
    
// //      while( extension[i] ) 
// //   {
// //      putchar(tolower(extension[i]));
// //       i++;
// //   }  
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    method1 = strtok(line1, " ");
    request1 = strtok(NULL, " ");
    version1 = strtok(NULL, "\r\n");
    
    
    char method[strlen(method1) + 1];
    strcpy(method, method1);
    char request[strlen(request1) + 1];
    strcpy(request, request1);
    char version[strlen(version1) + 1];
    ...
    if(strcmp(method, "GET")!=0){  //Compares the string pointed to, by str1 to the string pointed to by str2.
        error(405);
    }
    if(strncmp(request, "/",1)!=0){ //Compares at most the first n bytes of str1 and str2.
        error(501);        
    }
    if(strchr(request, '"')!=NULL){   //This returns a pointer to the first occurrence of the character c in the string str, or NULL if the character is not found.
        error(400);        
    }
    ...
    abs_path1 = strtok(request, "?");
    query1 = strtok(NULL, "\0"); 
    
    if(query1 != NULL){
        strcpy(abs_path, abs_path1);
        abs_path[strlen(abs_path1)] = '\0';
        strcpy(query,query1);
        query[strlen(query1)] = '\0';
    }
    else{
